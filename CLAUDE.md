...
- Open ⚠️ decisions for Mohamed: (1) **RESOLVED** — docs/02 §6a now normatively specifies Ma & Chen's broadcast model, cited. Still open: should **docs/02 §6**'s airtime constants adopt exact OFDM symbol quantisation? They are **0.41 % short** on data and **12.1 % on an ACK**; the NS-3 path already uses the exact form, but these constants feed `models.energy` → a ~0.4 % shift in E5's energy column and a deliberate re-freeze (audit A1, decision D9). (2) **docs/04 §1's 5–15× x86→ARM anchor** is unreachable (clock floor 2.61×) — restated per-cycle, needs blessing. (3) `[VERIFY]` citations in the paper. D6 wire freeze active.
