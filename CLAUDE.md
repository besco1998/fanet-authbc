...
- P7b live status (2026-07-27): **RPi4-A provisioned** — pyenv 3.12.13, `make test` 849 passed **on ARM** (blspy risk closed). First micro run flagged `.THROTTLED` (`get_throttled=0xe0000` = sticky freq-cap/throttle/soft-temp; under-voltage bit CLEAR ⇒ **cooling, not power**) → needs heatsink+fan **and a reboot** (sticky bits clear only on reboot), then re-run + `hw/compare_platforms.py`. 1× INA219 wired to Arduino Uno at 0x40, sketch uploaded. Pending: 2nd INA219 (bridge A0 → 0x41), RPi4-B, 10 kΩ pulldown on D2, WSL2 `usbipd attach`.
