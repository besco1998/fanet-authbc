...
- Phase: **P7b tooling COMPLETE, awaiting the physical rig** (tags …`e5-done`, `p7a-done`). ⚠D5 **settled = 2× INA219 + Arduino meter-host (Path A)**. Written & lint-clean: `hw/arduino/ina219_logger/` (50 Hz, sync-tagged CSV), `hw/energy_loop.py` (op driver, **in-process GPIO17 sync**, manifest), `hw/ina219_capture.py` (capture + `--reduce` → energy/op + CI; reduction verified vs synthetic data). Entry points: `make hw-capture`, `make hw-reduce`. Rig doc: `hw/RIG.md`. Also ready: **P8** paper draft (`paper/main.pdf`, citations marked `[VERIFY]`).
- Last green commit: main — 849 fast tests + 10 frozen-gate green; make lint clean. E5 headline stands: **T5 optimized cuts auth bytes 96.4% vs A+CBOR at V≥0.95, p=0.05 (PASS)**; energy nominal until P7b measurement.
- P7b live status (2026-07-27): **RIG LIVE, pi-A MEASURING.** pi-A clean micro run (arm64, pyenv 3.12.13, governor=performance, throttled=0x0, 1800 MHz) -> `results/hw/p1_*.authbc-pi4a.csv`. **F6 CONFIRMED: ECDSA wins on x86 (78.5 vs 95.0 us) but Ed25519 wins on ARM (259.5 vs 326.9 us)** -> co-design optimum picks Ed25519 (=D2 default) on the real platform; **96.4 % headline UNCHANGED** (both 64 B). Sizes byte-identical x86/ARM. **p_cpu_w measured ~0.73 W vs nominal 3.0 W (4.1x high)**. Full 8-op x 5-rep x 60 s energy campaign running. Audit: `docs/audits/p7.md`.
- ⚠ **pi-B BLOCKED (Mohamed):** under-voltage — clock collapses 1800->600 MHz under load, throttled=0x50005. Needs 5.2 V / shorter-thicker feed / own supply. Blocks **p_radio_w**, so the E5/E4 re-run + re-freeze is deliberately deferred (keeps E5's energy column all-nominal rather than half-measured).
- ⚠ **Spec-anchor finding for Mohamed:** docs/04 §1's 5-15x x86->RPi4 band is unreachable for primitives equally optimised on both platforms (clock floor is only 2.61x; Ed25519 needs just 1.05-1.16x the CYCLES). Restate the anchor per-cycle. Band NOT silently widened.
- Hardware facts: 2× RPi4 B (primary), 2× RPi3 B/B+, 2× BBB (stretch), 2× INA219 (0x40/0x41), Arduino. **Bookworm 64-bit Lite + pyenv 3.12** (match the x86 3.12.3 baseline; NOT Trixie's newer Python). 10 kΩ pulldown on Arduino D2 required; WSL2 needs `usbipd` to see the Arduino; PEP 668 ⇒ install into the repo venv.
...
- Known issues (full audit docs/audits/full_audit_pre_p7b.md, 2026-07-05): **F1 FIXED** (E4/T4 modelled BLS 48→96 B; conclusion Ed25519-wins unchanged/strengthened). Documented/open: F2 energy uses unicast T_FX ~1% over-count (fix at P7 energy re-run); F3 E5 b=28 grid-quantized (b_max=31, PASS either way); F4 cross-experiment record sizes differ (single-seed 68.9 vs 30-seed 66.25 cbor — random-walk sampling, standardize at P8); F5 32B prev_hash/record (stated assumption); F6 ECDSA<Ed on x86 (OpenSSL asm) may flip on ARM at P7b; F7 NS-3 broadcast gap = capture (unicast ±5% is the validation). Minor: e4_crossover.png missing from PROVENANCE.md (P8).
