...
- **LoRa arm SEPARATED (2026-07-28, docs/02 §9).** Its parameters are now sourced, not inherited: **Semtech SX1276 Rev.7 §4.1.1.7** (time-on-air) and **LoRa Alliance RP002-1.0.3** (EU868 Table 8 data rates, Table 13 max payload **242 B** non-repeater — the old "M=222" was Table 12, *repeater-compatible*; duty cycle **<1 %**). `models/lora.py` + 13 spec-pinned tests; `make exp-lora` → `results/raw/lora_eu868.csv`. **Λ and freshness are DERIVED from the duty cycle, not configured** — a frame of airtime T repeats every T/duty, so duty cycle fixes *both*. This is a **third regime** (MTU / freshness / duty-cycle) that T2a did not have. **Two findings: (1) at DR0–DR3 the 104 B of per-frame overhead (H_f 40 + sig 64) EXCEEDS the regional payload limit — the four longest-range LoRa modes cannot carry an authenticated frame at all; (2) sustainable Λ = 0.076 rec/s at DR5 (a record every 13 s) — 260× below 802.11's 20 rec/s — and freshness 39.4 s = 158× the 250 ms budget.** F5 (one chain hash per frame) is worth **2.7× the sustainable rate** here vs 26 % airtime on 802.11.
- **LoRa arm is now an OPTIMIZATION, not a feasibility study (docs/02 §9a).** `models/lora_codesign.py` + `make exp-lora-codesign` → `results/raw/lora_codesign.csv`. Same method as E5 (exhaustive, Pareto, nothing hand-picked) with two structural differences: **(i) the data rate is a 5th design variable**, with range entering as a *ratio* from the SX1276 sensitivity table (DR0 −136 dBm = **6.31×** the range of DR6 −120 dBm) — without that objective DR6 dominates on Λ and D and the DR axis collapses, which a test now pins; **(ii) Λ and freshness are DERIVED from the duty cycle, not inputs** (Λ = b·duty/T, D = T/duty), so a bigger batch raises Λ but *worsens* freshness — the LoRa co-design tension, with no 802.11 counterpart. **Result: 3414 of 3696 points do not physically exist; 282 feasible; 108 Pareto across DR4/5/6.** Doubling range costs ~3.5× the record rate. ⚠️ **No energy column** — the only measured p_radio_w is a Wi-Fi figure; a LoRa energy number needs a LoRa measurement.
- **F12 FIXED + channel capacity is now a constraint (2026-07-28).** docs/01 §5 defines Λ = Λ_i·N_local, but the optimizer used one value for both: freshness (per-node, correct) and verify-throughput (aggregate — **under-counted by N_local**, silently testing a one-sender network). Fixed via `Constraints.lam` + `n_local` + derived `lam_aggregate`. **It changed T4:** counting the real aggregate, placement C dies at N≥10 and **BLS is entirely infeasible beyond ~17-25 neighbours** — Ed25519 doesn't just win on energy, BLS is *not verifiable* at fleet scale.
- **Λ_i = 20 rec/s is now SOURCED** (docs/01 §1): an AUTHBC record = MAVLink `GLOBAL_POSITION_INT` + part of `SYS_STATUS`; PX4 streams that at 5 Hz (telemetry radio) / 10 Hz (OSD,USB) / **50 Hz (`MAVLINK_MODE_ONBOARD`, companion computer)**; ArduPilot's 1 Hz is its 57k SiK default. 802.11 is a companion-class link so 20 Hz is between OSD and ONBOARD — **not inflated**. Coupling recorded: batching needs **Λ_i ≥ 1/D_max** (=4 rec/s at 250 ms).
- **NEW RESULT — the baselines cannot physically run at fleet scale.** `make exp-capacity` → `results/raw/capacity_envelope.csv` (docs/02 §6b). At N_local=50, Λ_i=20: **A+JSON U=2.28, A+CBOR U=1.53 (both over capacity), optimized U=0.55.** The co-design is the difference between working and not working at 50 UAVs — a stronger claim than "75 % fewer auth bytes". PX4's 50 Hz companion rate is supported to ~50 UAVs. ⚠️ **Throughput envelope only — DCF access delay is NOT modelled**, so D(b) is credible only at low U; `channel_util` is now reported beside it.
- Known issues: **FIXED/CLOSED — F1, F3, F4, F8, F9, F10, F11, D9, the docs/04 anchor, and audit items P1/P2/P3 + A0–A12.** **F6**: settled as *report the measurement, claim no cause*. **F5 investigated, decision pending:** the 32 B chain hash is **71 % of a delta record**, and within a frame every prev_hash but the first is **derivable** — proven byte-identical by test. Adopting one-link-per-frame would cut on-air bytes **71.0 → 47.0 B/record (−34 %)** and raise φ 58.7→75.3 %, headline unchanged at 75.00 %. ⚠️ Wire format is D6-frozen ⇒ Mohamed's call (`docs/audits/model_provenance.md` F5). **⚠️ `[VERIFY]` citations** deferred to the start of P8, when the whole literature set gets verified at once.
