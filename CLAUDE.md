...
- **D1 CLOSED BY MEASUREMENT (INA219, pi-A, 5 reps/config).** Model under-predicts sender-side CPU energy **~32 %**: optimized 44.25→**58.38** µJ/rec (+31.95 %), baseline 87.83→**118.83** (+35.30 %). Half omitted chain-hash + framing (hash = 4.64 µs/rec on ARM), half a single `p_cpu_w` understating a composed pipeline (0.732 vs 0.634 W). **Absolute energy is a LOWER BOUND; relative comparisons survive** (bias near-uniform).
