...
\emph{quantification} of the resulting dispersion. Of the \emph{five} ns-3 LoRa simulation studies
now meeting our pre-registered inclusion criteria, four state no seed count, confidence interval or
standard deviation that would let a reader detect it. \textbf{The fifth does}: Klimiashvili
\emph{et al.} average every reported result over ``50 independent runs over channel realization and
nodes' position''~\cite{klimiashvili2020loravswifi}. We report that as a count and not a
percentage, because five papers cannot support one: the protocol targeted 56, the achieved $n$ is
$5$, and a single counter-example moves the estimate from $0\%$ to $20\%$. What this supports is
the weak claim --- that a reader of this literature frequently \emph{cannot} tell --- not a
quantitative claim about how often. That is why every capacity figure in this
