...
$1.9\times$ and $3.3\times$ the swarm of an inline baseline on the same medium, depending on the
...
\toprule
configuration & $\Lambda$ & $D_{\max}$ & $b$ & \multicolumn{2}{c}{$N_{\max}$} \\
 & (rec/s) & (ms) & & $U{<}1$ & $V{\geq}0.95$ \\
\midrule
\multicolumn{6}{l}{\emph{Primary: compliant with 3GPP TS~22.125}~\cite{3gpp_ts22125}} \\
\textbf{optimized} delta/B & 50 & 100 & 4 & \textbf{35} & \textbf{116} \\
A+CBOR (Pillar-1) & 50 & 100 & 1 & 18 & 35 \\
\midrule
\multicolumn{6}{l}{\emph{Relaxed freshness: same bytes, larger swarm, exceeds the standard}} \\
optimized delta/B & 20 & 250 & 4 & 103 & 233 \\
A+CBOR (Pillar-1) & 20 & 250 & 1 & 32 & 104 \\
A+JSON (naive) & 20 & 250 & 1 & 25 & 65 \\
\midrule
\multicolumn{6}{l}{\emph{Compliant but below the batching threshold} ($\Lambda D_{\max} < 4$)} \\
optimized delta/B & 20 & 100 & 1 & 34 & --- \\
optimized delta/B & 10 & 100 & 1 & 78 & --- \\
\bottomrule
\end{tabular}
\end{table}
...
$1.9\times$ and $3.3\times$ (Table~\ref{tab:envelope}): $1.94\times$ and $3.31\times$ at the two
thresholds for the compliant point, $3.22\times$ and $2.24\times$ for the relaxed one. \textbf{The
...
swarm size, $233 \to 116$ at the $V{\geq}0.95$ boundary and $103 \to 35$ at saturation. The relaxed
...
threshold raises delivery by $3.3$ points at $N{=}8$ and by $1.36\times$ at $N{=}50$ without moving
...
$(1-2\tau/T)^{N-1} = 0.868$ at $N{=}8$ against $0.866$ measured.
...
$2.68\times$ at $N{=}50$ ($0.2532 \rightarrow 0.6781$) and still $0.5308$ at $N{=}100$, which the
peer configuration cannot reach at all. \textbf{Yet $N_{\max}$ moves only from 5 to 8.} The
$V \geq 0.95$ criterion sits on a near-vertical part of the delivery curve --- the gateway passes at
$N{=}8$ with $0.9958$ and fails at $N{=}10$ with $0.9370$ --- so a threefold reduction in offered
...
100\,ms bound of 3GPP TS~22.125~\cite{3gpp_ts22125} costs swarm size---$N{\leq}116$ instead of
$N{\leq}233$---and nothing in bytes, provided the telemetry rate is high enough for the batch to
