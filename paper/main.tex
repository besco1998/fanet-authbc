...
\emph{quantification} of the resulting dispersion. Of the \emph{fourteen} ns-3 LoRa simulation studies
meeting our pre-registered inclusion criteria, \textbf{twelve} state no seed count, confidence
interval or standard deviation that would let a reader detect it. \textbf{Two do}, and both are
counter-examples to our own hypothesis: Klimiashvili \emph{et al.} average over ``50 independent
runs over channel realization and nodes'
position''~\cite{klimiashvili2020loravswifi}, and Khan \emph{et al.} collect theirs ``from 100
simulation runs''~\cite{reliableuplink2019}. At $14.3\%$ reporting, against a falsification
threshold of $25\%$ fixed before the corpus existed, the hypothesis is supported --- but the
achieved $n$ is $14$ where the protocol targeted $56$, and we report it as the achieved $n$.
The shortfall is not neutral: retrieval succeeded on arXiv and failed on the paywalled venues,
so the corpus is biased toward preprints, and whether that population reports better or worse is
something these fourteen papers cannot say.

Hand adjudication was not a formality. A purely mechanical sweep would have been wrong on three
papers, most instructively by matching \texttt{seed} against ``Seed Studio'' --- a hardware
vendor's name --- and scoring a paper as reporting replication on the strength of a component
supplier. That is why every capacity figure in this
