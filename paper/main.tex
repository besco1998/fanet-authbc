...
configuration reaches $N{=}103$---a ${\approx}3\times$ larger swarm, a ratio that holds whether the
limit is taken at saturation throughput or at the measured verifiability boundary. Because a tighter freshness deadline
shrinks the batch and so raises the frame rate, latency and capacity trade against each other: meeting
the 100\,ms bound of 3GPP TS~22.125 costs a factor of two in supportable swarm size and nothing in
bytes. Freshness, not the MTU, is what binds:
...
validated against NS-3 3.48: unicast matches Bianchi within $+1.28$/$-0.49\%$, while the common
reduction of that model to broadcast fails by $16\times$ at $N{=}50$ and is replaced by Ma and Chen's
broadcast model~\cite{machen2007,machen2008}, which our measurements confirm to within $2.49\%$. All results are reproduced deterministically from frozen data by
...
\subsection{T6: authentication-exclusion threshold}\label{sec:t6}
T1--T5 ask what a saved byte buys. T6 asks the prior question --- whether a link admits
authenticated telemetry at all. A frame that is independently verifiable must carry the header, the
...
$n{=}1$ and is an exclusion, not an inconvenience.
...
(802.11a 6\,Mb/s, broadcast model of~\cite{machen2007}). Computed against saturation throughput
($U<1$), which simulation shows to be conservative by ${\approx}2.8\times$; the measured
$V{\geq}0.95$ boundary gives 65 / 104 / 233 for the same three rows. The ratio, not the absolute
limit, is the claim.}
\label{tab:envelope}
...
are reported because they answer different questions. Against \emph{saturation} throughput ($U<1$)
the naive and Pillar-1 baselines exhaust a single collision domain at 25 and 32 neighbours while the
co-design reaches 103. That criterion is conservative: saturation throughput is what the medium
carries when every node is backlogged, and simulation shows $98.8\%$ delivery still at $U{=}1$, with
verifiability crossing $V{=}0.95$ only near $U{\approx}2.8$. Against \emph{that} measured boundary
the same three configurations reach 65, 104 and 233. The ratio is what survives both readings: the
co-design carries ${\approx}3\times$ the swarm on the same medium. This statement needs the encoding
(frame size), the placement and batch (frames per second), the scheme (auth bytes) and the channel
model together; no single axis produces it.
...
are pure ALOHA, with no carrier sense and no acknowledgement. Simulating the AUTHBC frame at DR5
under the same $V \geq 0.95$ criterion used for 802.11 gives $N_{\max} = 5$, with a sharp cliff:
delivery is $1.000$ at $N{=}5$ and $0.866$ at $N{=}8$. There is no backoff to absorb contention.
...
channel still delivering $98.8\%$ of frames at nominal saturation load, so meeting the
100\,ms bound of 3GPP TS~22.125~\cite{3gpp_ts22125} costs swarm size---$N{\leq}116$ instead of
$N{\leq}233$---and nothing in bytes. An NS-3-validated channel model and an automated
