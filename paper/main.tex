...
\emph{placement}, signature \emph{scheme}, and \emph{batch} size---must be chosen jointly, and a
factorial ablation identifies which of them actually interact: placement and batching are
inseparable (placement saves exactly $g_a(1{-}1/b)$, hence \emph{nothing} at $b{=}1$), encoding is
additive, and the scheme axis is byte-neutral at the operating point. We formalize this with five results (overhead dominance, batching amplification, a
...
sink-directed shape~\cite{abrardo2019multihoplora,jiang2020hybridlpwan}. Our scenario is therefore $N$ end devices reporting to one
...
networks~\cite{sobati2025fragmentsec}, so the change is not only a byte-accounting one.
