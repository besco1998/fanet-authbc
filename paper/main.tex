...
\emph{co-optimized}. We formalize this with five theorems (overhead dominance, batching amplification and its
regime boundary,
a loss-robustness frontier, a power-independent scheme crossover, and the co-design optimum), implement
...
optimizer selects a delta encoding under self-batch placement, cutting authentication overhead by
\textbf{75.0\%} (to $26.0$\,B/record) at $V = 0.95$ \emph{and} a 250\,ms freshness bound---criterion
met. Freshness, not the MTU, is what binds: the byte-optimal batch cuts 96.8\% but incurs 1.55\,s of
staleness, so a co-design optimizer blind to freshness solves the wrong problem. The airtime model is
...
All values are from frozen CSVs (x86 host, Intel i5-14400F; energy uses nominal power pending hardware
measurement).
...
(minimum $\kappa^\ast = 3.21 \gg 0.5$). At 96\,B, BLS carries more auth bytes than Ed25519 on own
self-batch traffic ($\kappa^\ast=\infty$) and saves bytes only on relay traffic for $b\geq 2$; its
$10.7\times$ verify cost keeps Ed25519 optimal for any plausible power ratio.
...
Auth cut $=(104-26.0)/104=75.0\% \geq 40\% \Rightarrow$ PASS.}
\label{tab:e5}
\begin{tabular}{llll r r r}
\toprule
config & enc & $\sigma$ & place & auth B/rec & $V$ & $D$ (ms) \\
\midrule
\textbf{optimized} & delta & Ed & B,$b{=}4$ & \textbf{26.00} & 0.95 & 200 \\
...
self-batch at $b{=}4$, cutting on-air authentication from 104\,B/record to \textbf{26.0\,B/record}---a
\textbf{75.0\%} reduction versus the Pillar-1 baseline at $V=0.95$, $p=0.05$ and a 250\,ms freshness
bound. Hand-checked: $45.0+104/4=71.0$\,B/record, freshness $4/20=200$\,ms.

\textbf{Which constraint binds is itself the result.} Ignoring freshness, the byte-optimal batch is
$b{=}31$ at the MTU knee, cutting 96.8\%---but the oldest record in such a batch waits
\textbf{1.55\,s}, $6.2\times$ over $D_{\max}$. Because fill time dominates the delay, the admissible
...
optimizing encoding, placement, scheme, and batching cuts on-air authentication by 75.0\% at
$V\geq0.95$---meeting the falsifiable criterion---while an NS-3-validated channel model and an automated
