...
\emph{composed} from separately measured powers and timings, and we metered the composition
end-to-end: it under-predicts real sender-side CPU energy by ${\approx}32\%$ (44.25 to a measured
58.38\,\micro J/record at $b{=}4$; 87.83 to 118.83 at $b{=}1$). Half of that is an omitted chain-hash
and frame-assembly term, half is that a single $P_\mathrm{cpu}$ measured over isolated primitives
understates a composed pipeline (0.732 versus 0.634\,W). The bias is near-uniform across
configurations, so \emph{relative} energy comparisons hold---the measured advantage (2.035$\times$)
slightly exceeds the predicted one (1.985$\times$)---but absolute energy values should be read as
lower bounds. The channel models are
