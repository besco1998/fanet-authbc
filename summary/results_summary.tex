...
% ⚠️ SUPERSEDED 2026-07-29. This running summary predates the pre-P8 audit and carries
% stale numbers (H_f=40, the four-axis headline framing, ns-3.41 bands, nominal-era energy).
% It is kept for provenance only. The current account lives in paper/main.tex and the
% docs set indexed by docs/README.md. Do not cite from this file.
\begin{document}
\maketitle
\thispagestyle{fancy}
...
\subsection{T2a: where ``compression pays $\times A$'' actually applies}
T2's amplification $A=M/(M-H_f-g_a)$ is derived \emph{at the MTU limit}. Under the freshness bound
the batch is capped at $\lfloor\Lambda D_{\max}\rfloor$ instead, which does not depend on the record
size --- so $dC/ds=1$ \textbf{exactly} and compression pays $1\times$. E2's MTU sweep straddles the
boundary:
\begin{center}\begin{tabular}{rrrl r}
\toprule MTU & $b_{\max}$(MTU) & ceiling & binds & $A$ realised \\ \midrule
256 (LoRa-like) & 3 & 3 & MTU & \textbf{1.684} \\
576 & 10 & 5 & freshness & 1.000 \\
1500 (802.11) & 31 & 5 & freshness & 1.000 \\ \bottomrule
\end{tabular}\end{center}
On 802.11 the MTU knee is never reached, so $A$ never operates; on a low-rate link it does. The
leverage motivating the LoRa arm is real \emph{and exclusive to it}.
