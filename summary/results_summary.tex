...
on-air authentication bytes by \textbf{96.8\,\%} versus the inline-CBOR baseline while holding
...
\toprule $N$ & unicast vs ACK-Bianchi & broadcast vs no-ACK Bianchi & broadcast vs slot-exact \\ \midrule
5 & $+0.08$\,\% & $-0.5$\,\% & $-0.62$\,\% \\
20 & $+0.14$\,\% & $+30.8$\,\% & $-1.10$\,\% \\
50 & $\mathbf{-3.31}$\,\% & $\mathbf{+1647.9}$\,\% & $\mathbf{+0.79}$\,\% \\ \bottomrule
\end{tabular}\end{center}
Unicast validates the Bianchi DCF model to $+0.08$--$-3.31\,\%$ (tightened from the previously
reported $\pm1.8$--$5.3\,\%$ by fixing F8, a $\sim$4.8\,\% goodput over-count in which the NS-3 sinks
outlived the sources by 0.5\,s on a 10\,s window). The broadcast column fails not because of capture
--- measured at \textbf{0\,\%} --- but because the no-ACK Bianchi variant omits the
\emph{post-transmission head start}: a station that has just transmitted may redraw backoff 0 and
seize the medium one slot before any deferring station, whose counter is necessarily $\ge1$. With
broadcast's frozen contention window that channel dominates at high $N$. A slot-exact simulation
adding only this asymmetry reproduces NS-3 to $\le1.1\,\%$ at every $N$ (F9).
...
architectural default). Both are 64\,B, so the 96.8\,\% headline is \textbf{unchanged}.
...
busy periods, and an equal-power channel is byte-identical to the default. The measured cause is the
post-transmission head start; a slot-exact model carrying only that asymmetry matches NS-3 to
$\le1.1\,\%$, while reproducing Bianchi exactly when the asymmetry is switched off. \\ \addlinespace
