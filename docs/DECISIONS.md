...
| **D5** ⚠️ | energy meter: **INA219** (was UM25C) | you own it; I2C-scriptable > eyeballing a USB meter | shunt **burden voltage** can under-volt an RPi4; measures **whole-board** (not per-component) power | 5.1 V+ supply, low/known shunt, calibrate vs a known load, log from a **separate** meter-host; optional INA260 removes the burden issue. **Confirm to close D5.** |
...
| **Broadcast gap = capture effect** (18× over-claim retracted, then confirmed via power-spread) | honest root-cause, not a fudge factor | the no-ACK broadcast Bianchi is only a **lower bound** — broadcast can't be *quantitatively* validated by it | unicast (±5 %) is the quantitative DCF validation; broadcast is qualitative + capture-confirmed |
