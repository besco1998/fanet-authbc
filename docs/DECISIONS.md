...
| **Airtime is OFDM-symbol quantised** (⚠️ **D9**, settled 2026-07-28 by Mohamed) | the continuous 8N/R form was **0.41 %** short on a 1400 B frame and **12.1 %** short on an ACK against NS-3 3.41, and used 34 B MAC overhead instead of the real 36 B (audit A1) | airtime is a **step** function of L, so `T_fx` and the affine `T_air(L)` are gone; E4's ΔRADIO stays continuous (±5 % on a byte difference, immaterial against a ~90× verdict margin) | applied everywhere (NS-3 path, `models.energy`, `channel.airtime`); deliberate re-freeze: **E5 energy +0.096 %** (52.1487→52.1985 µJ), **E3 goodput −2.1 %**; **auth-byte headline unchanged at 96.77 %, PASS** |

| **Freshness reported, not filtered** (⚠️ **F10**, open — Mohamed to decide) | docs/02 §7's D_max=250 ms is a *soft* bound and the ≥40 % criterion never mentioned freshness; the batching/freshness trade-off is itself a co-design result | the byte-optimal b=31 sits at **1552 ms — 6.2× over** the bound, and E5 computed `meets_latency` then **discarded it**, so the cost was invisible; only 160/521 feasible configs meet it | now emitted as `latency_ms` + `meets_d_max` in `e5_codesign.csv`. **Options:** keep soft (headline 96.77 %); make hard (optimum b=4, cut **75.00 %**, 200 ms, 111.9 µJ — still PASS); or revise D_max |
