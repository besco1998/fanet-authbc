...
| **Broadcast gap = post-transmission head start** (F9, 2026-07-28) — *supersedes two retracted explanations: the "18× capture" over-claim and, after it, "the capture effect" itself* | measured, not argued: PHY traces put capture at **0 %** (all 1149 decodes came from single-transmitter busy periods; `--equalPower` is byte-identical), while the gap is fully reproduced by one asymmetry — a station that just transmitted may redraw backoff 0 and take the medium a slot before any deferring station (counter ≥ 1) | the corrected broadcast model is a **slot-exact simulation** (`sim/dcf_ladder.py`), not closed form; the textbook no-ACK Bianchi variant is wrong by up to 16× and is retained only as the failing reference | broadcast is now validated to **≤1.1 %** across N=5–50; ⚠️ whether docs/02 §6 adopts the slot-exact process as normative is Mohamed's call |
