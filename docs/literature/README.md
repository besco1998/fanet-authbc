...
| source | status | role |
|---|---|---|
| **Bianchi, *Performance analysis of the IEEE 802.11 DCF*, IEEE JSAC 18(3), 2000** | `USED` | The unicast saturation model our airtime layer implements. Validated against NS-3 3.48 to **+1.28/−0.49 %** |
| **Ma & Chen, *Saturation Performance of IEEE 802.11 Broadcast Networks*, IEEE Comm. Lett. 11(8), 2007** — PDF here | `USED` `PRIOR ART` | **Cost us a novelty claim (F9).** We independently rediscovered the backoff-counter freeze that makes broadcast differ from unicast; they published it in 2007. Their abstract explicitly warns unicast models "cannot simply be reduced" to broadcast — which is exactly what we had done, wrong by 16× at N=50. Now implemented and cited; no novelty claimed |
